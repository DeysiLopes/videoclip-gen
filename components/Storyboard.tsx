...
const SceneCard: React.FC<SceneCardProps> = ({
  ...
  const renderStatusBadge = () => {
     if (isApproved) {
       return <div className="absolute top-2 right-2 bg-green-500/20 text-green-300 text-xs font-bold px-2 py-1 rounded-full border border-green-500 flex items-center gap-1">
  ...
  return (
    <div className={`bg-gray-800/60 rounded-xl border ${isApproved ? 'border-green-500/50' : 'border-gray-700'} shadow-lg transition-all`}>
      <div className="p-4">
        <div className="flex justify-between items-center mb-3">
          ...
          <div className="flex items-center gap-2 bg-gray-900/50 border border-gray-700 rounded-md px-2 py-1">
            ...
            {isEditing ? (
              <input type="text" value={timestampStr} onChange={e => setTimestampStr(e.target.value)} className="w-16 bg-transparent text-center font-mono text-sm" />
            ) : (
      ...
      <div className="bg-black/30 aspect-video flex items-center justify-center overflow-hidden">
        ...
        {scene.videoUrl && <video src={scene.videoUrl} muted loop autoPlay playsInline className="w-full h-full object-cover" />}
      ...
      <div className="p-3 bg-gray-900/30 rounded-b-xl flex items-center justify-between gap-2">
        <div>
          {!isEditing && (
            <button onClick={() => setIsEditing(true)} className="p-2 text-gray-400 hover:text-white hover:bg-gray-700 rounded-full"><EditIcon className="w-4 h-4"/></button>
          )}
          <button onClick={() => onDelete(scene.id)} className="p-2 text-gray-400 hover:text-red-400 hover:bg-gray-700 rounded-full"><DeleteIcon className="w-4 h-4"/></button>
        </div>
        <div>
        {isEditing ? (
    ...
    <div className="w-full h-full flex flex-col">
      {projectConfig.audioUrl && (
        <div className="mb-4 p-4 bg-gray-800 rounded-xl border border-gray-700 w-full flex-shrink-0">
          ...
          <audio ref={audioRef} src={projectConfig.audioUrl} controls className="w-full" />
        </div>
      )}
